...
class MonoDepth(SegBaseModel):
    ...
    def __init__(self, backbone='resnet50', ctx=cpu(), pretrained_base=True,
        ...
        with self.name_scope():
            up_channels = [256, 512, 1024, 2048]
            strides = [4, 8, 8, 8] if dilated else [4, 8, 16, 32]
            ...
            for i in range(2, -1, -1):
                inter_channels = up_channels[i]
                h, w = height // strides[i], width // strides[i]
                self.upconvs.add(_UpConv(up_channels[i+1], inter_channels, 3, h, w, **kwargs))
                self.connects.add(_ConvBnAct(up_channels[i]+inter_channels, inter_channels, **kwargs))
                self.disps.add(_DepthHead(inter_channels, 2, **kwargs))
            #self.head = _DepthHead(2048, 2, **kwargs)
    ...
    def hybrid_forward(self, F, x):
        ...
        for i in range(len(skips)):
            ...
            if i > 1:
                disps.append(self.disps[i-2](feat))
    ...
    def hybrid_forward(self, F, x):
        ...
        for i in range(len(features) - 1):
            ...
            xi.wait_to_read()
            upfeat = self.upconvs[i](feat)
            feat = self.connects[i](F.concat(upfeat, xi, dim=1))
            feat.wait_to_read()
            disps.append(self.disps[i](feat))

        strides = [1, 2, 2, 2] if self.dilated else [1, 2, 4, 8]
        outputs = [(F.contrib.BilinearResize2D(x, self._up_kwargs['height'] // stride,
                                               self._up_kwargs['width'] // stride)) \
                   for x, stride in zip(disps, strides)]
        # L-R disps
